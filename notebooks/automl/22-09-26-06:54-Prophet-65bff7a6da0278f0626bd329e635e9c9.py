...
# MAGIC - Compare trials in the [MLflow experiment](#mlflow/experiments/2662581313052873/s?orderByKey=metrics.%60val_smape%60&orderByAsc=true)
# MAGIC - Navigate to the parent notebook [here](#notebook/2662581313052874) (If you launched the AutoML experiment using the Experiments UI, this link isn't very useful.)
...
input_data_path = input_client.download_artifacts("389a97e908394425a2535fc50ae25444", "data", input_temp_dir)
...
# MAGIC - All the runs are logged under [this MLflow experiment](#mlflow/experiments/2662581313052873/s?orderByKey=metrics.%60val_smape%60&orderByAsc=true)
...
df_aggregation = df_aggregation.assign(ts_id=lambda x:x["store"].astype(str)+"-"+x["item"].astype(str))
...
def prophet_training(history_pd):
  ...
  hyperopt_estim = ProphetHyperoptEstimator(horizon=horizon, frequency_unit=unit, metric="smape",interval_width=0.95,
                   country_holidays=country_holidays, search_space=search_space, num_folds=2, max_eval=1, trial_timeout=168,
                   random_state=296567059, is_parallel=run_parallel)
...
with mlflow.start_run(experiment_id="2662581313052873", run_name="PROPHET") as mlflow_run:
